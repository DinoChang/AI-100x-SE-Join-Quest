...
using ChineseChess.Specs.Support;
...
public class ExtentReportHooks(ScenarioContext scenarioContext, FeatureContext featureContext)
{
    ...
    public static void BeforeTestRun()
    {
        ...
        Console.WriteLine("ChineseChess ExtentReports initialized");
    }
    ...
    public static void BeforeFeature(FeatureContext featureContext)
    {
        ...
        Console.WriteLine($"ChineseChess Feature started: {featureName} (Key: {featureKey})");
    }
    ...
    public void BeforeScenario()
    {
        ...
        Console.WriteLine($"ChineseChess Scenario started: {scenarioName} (Key: {scenarioKey})");
    }
    ...
    public void AfterScenario()
    {
        ...
        if (_scenarioContext.TestError != null)
        {
            ...
            Console.WriteLine($"ChineseChess Scenario failed: {scenarioName}");
        }
        else
        {
            ExtentTestReporter.LogStep(scenarioKey, Status.Pass, $"Scenario: {scenarioName}", "Scenario completed successfully");
            Console.WriteLine($"ChineseChess Scenario passed: {scenarioName}");
        }
    }
    ...
    {
        var featureName = featureContext.FeatureInfo.Title;
        Console.WriteLine($"ChineseChess Feature completed: {featureName}");
    }
    ...
    public static void AfterTestRun()
    {
        ...
        {
            var reportFiles = Directory.GetFiles(testResultsDir, "ChineseChess_ExtentReport_*.html");
            if (reportFiles.Length > 0)
            {
                var latestReport = reportFiles.OrderByDescending(File.GetCreationTime).First();
                Console.WriteLine($"ChineseChess ExtentReports report generated: {latestReport}");
                ...
                if (Environment.GetEnvironmentVariable("CI") == null)
                {
                    ...
                    {
                        Console.WriteLine($"Could not open ChineseChess report automatically: {ex.Message}");
                    }
                }
            }
        }
        ...
        Console.WriteLine("ChineseChess ExtentReports completed and cleaned up");
    }
}